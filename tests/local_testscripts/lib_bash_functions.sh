...
function cleanup() {
  ...
  rm -f  /usr/local/bin/lib_shopware6_api_base
  cd "${save_dir}" || exit
...
function test_commandline_interface_venv() {
  ...
  if ! "/usr/local/bin/lib_shopware6_api_base" --version; then
...
function test_commandline_interface_venv_old() {
  ...
  clr_green "issuing command : $HOME/venv/local/bin/lib_shopware6_api_base --version"
  if ! "$HOME/venv/local/bin/lib_shopware6_api_base" --version; then
    my_banner_warning "test commandline interface on virtual environment ERROR"
    beep
    sleep "${sleeptime_on_error}"
    return 1
